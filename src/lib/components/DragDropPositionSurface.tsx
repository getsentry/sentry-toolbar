...
export default function DragDropPositionSurface({instanceName}: Props) {
  ...
  return (
    <Fragment>
      <ConfigPositionDropTargets />
      <div
        className={cx(grabberGhostClass, ghostPositionClass({position: lastPosition as Configuration['placement']}))}
...
function useDragDropPositionSurface({onPositionChange}: {onPositionChange: (position: string) => void}) {
  const {shadowRoot, reactMount} = useContext(DomNodeContext);

  const [mousePosition, setMousePosition] = useState<null | Coord>(null);
...
function ConfigPositionDropTargets() {
  return (
