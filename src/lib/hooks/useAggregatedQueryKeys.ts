...
interface Props<AggregatableQueryKey, Data> {
   ...
   */
  getQueryKey: (ids: readonly AggregatableQueryKey[]) => ApiEndpointQueryKey;
...
function isQueryKeyInList<AggregatableQueryKey>(queryList: AggregatableQueryKey[]) {
  return ({queryKey}: {queryKey: QueryKey}) => queryList.includes(queryKey[4] as AggregatableQueryKey);
}
...
export default function useAggregatedQueryKeys<AggregatableQueryKey, Data>({
  ...
  const {fetchDataFn} = useSentryApi();
  ...
  const readCache = useCallback(
    () =>
      cache
        ...
        .map(({queryKey}) => queryClient.getQueryData<ApiResult<CountState>>(queryKey))
  ...
  const fetchData = useCallback(() => {
    ...
    const queuedAggregatableBatch = queuedQueriesBatch.map(({queryKey}) => queryKey[4] as AggregatableQueryKey);
    ...
    try {
      ...
      queuedAggregatableBatch.forEach(queryKey => {
        queryClient.setQueryData<boolean>(['aggregate', cacheKey, key, 'inFlight', queryKey], true);
      });
  ...
  }, [bufferLimit, cache, cacheKey, fetchDataFn, getQueryKey, key, onError, queryClient]);
  ...
  const buffer = useCallback(
    (aggregates: readonly AggregatableQueryKey[]) => {
      ...
      const queryKeys = [...new Set([...prevQueryKeys.current, ...aggregates])];
    ...
    [bufferLimit, cache, cacheKey, clearTimer, fetchData, key, queryClient, readCache, setTimer]
  );
